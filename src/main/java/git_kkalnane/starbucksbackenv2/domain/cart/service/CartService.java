    ...
    public class CartService {
        ...
        public List<Long> deleteCartItem(DeleteCartItemDto deleteCartItemDto, Long memberId) {
            Cart cart = validAndCalculatorService.findCartByMemberId(memberId);
            List<Long> cartItemIds = deleteCartItemDto.cartItemId();
            ...
            List<Long> existingCartItemIds = cartItems.stream()
                    .filter(item -> item.getCart().getId().equals(cart.getId()))
                    .map(CartItem::getId)
                    .toList();

            cartItemOptionRepository.deleteAllByCartItemIds(existingCartItemIds);
            cartItemRepository.deleteAllById(existingCartItemIds);

            return existingCartItemIds;
        }
        ...
        public CheckCartItemResponse getCartItem(Long memberId) {
            ...
            Long totalPrice = calculateCartTotalPrice(cart);
        ...
        public Long calculateCartTotalPrice(Cart cart) {
            return cart.getCartItems().stream()
                    .mapToLong(cartItem -> cartItem.getItemPrice() * cartItem.getQuantity())
                    .sum();
        }
